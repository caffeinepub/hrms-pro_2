...
actor {
  ...
  // V1 type (old) — used only to receive existing stable data during migration
  ...
  system func postupgrade() {
    ...
    // Migrate attendance V1 -> V2 (add inTime/outTime)
    if (not attendanceMigrated) {
      for (a in attendanceV1Records.values()) {
        attendanceRecords.add(a.id, {
          id = a.id;
  ...
  // V3 type with employeeType, inTime, outTime
  type Employee = {
    id : Text;
    name : Text;
    code : Text;
    ...
    joiningDate : Text;
    photo : Text;
    employeeType : Text;  // "Permanent" | "Casual"
    inTime : Text;
    outTime : Text;
  };
  ...
  type Attendance = { id : Text; employeeId : Text; date : Text; status : Text; inTime : Text; outTime : Text };
  module Attendance {
    public func compare(a : Attendance, b : Attendance) : Order.Order { Text.compare(a.id, b.id) };
  };
  let attendanceV1Records : Map.Map<Text, AttendanceV1> = Map.empty<Text, AttendanceV1>();
  ...
  public shared func markAttendance(attendance : Attendance) : async () {
    ...
    attendanceRecords.add(attendance.id, attendance);
  };

  public shared func updateAttendance(attendance : Attendance) : async () {
    if (not attendanceRecords.containsKey(attendance.id)) Runtime.trap("Attendance record not found");
    if (not employeesV3.containsKey(attendance.employeeId)) Runtime.trap("Employee not found");
    attendanceRecords.add(attendance.id, attendance);
  };
  ...
  public query func getAttendanceByDate(date : Text) : async [Attendance] {
    attendanceRecords.values().toArray().filter(func(a : Attendance) : Bool { a.date == date }).sort();
  };

  public query func getAttendanceByEmployee(employeeId : Text) : async [Attendance] {
    attendanceRecords.values().toArray().filter(func(a : Attendance) : Bool { a.employeeId == employeeId }).sort();
  };
